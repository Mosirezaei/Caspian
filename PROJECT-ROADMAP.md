...
- [x] مرحله ۵ — نوشتن محتوای ۲۴ صفحه — هر ۲۴ آیتم تکمیل شد (2026-09-03)
...
## نکته مستندسازی — وضعیت /retro (اضافه‌شده 2026-09-07)

`/retro` (فروش بلیط کنسرت Disko Legend / Retro Stage) یک صفحه‌ی مستقل و دائمیه، نه Event موقت.
تصمیم گرفته شده که این صفحه به همین شکل (مسیر اختصاصی خودش، جدا از `/events`) باقی بمونه و از صفحه اصلی
هم تبلیغ بشه. به همین دلیل عمداً `noindex` (متا تگ روی خود صفحه) داره ولی از robots.txt بلاک نیست و در
sitemap.xml هست — این یک انتخاب عمدیه، نه باگ. اگر Event موقت دیگه‌ای بعداً اضافه شد، باید جداگانه
تصمیم گرفت که مستقل بمونه یا زیر `/events` بره.
...
[این یادداشت 2026-09-02 بود و موقع نوشتنش هنوز ۳ مقاله (۱۷، ۲۰، ۲۲) باقی مونده بود؛ طبق چک‌لیست
بالا، هر سه در 2026-09-03 نوشته و تکمیل شدن — مرحله ۵ الان کامل ۲۴ از ۲۴ هست.]


---

## یادداشت جلسه ممیزی — 2026-09-08 (Composio/GitHub, بررسی کامل route/sitemap/کد یتیم)

⚠️ **این فایل دیگه مرجع کامل صفحات سایت نیست.** چک‌لیست «۲۴ صفحه» بالا مربوط به مرحله‌ی اولیه‌ی
بازطراحیه (تا 2026-09-03) و صفحات/تغییرات بعدی توش ثبت نشدن. وضعیت واقعی امروز (تأیید شده با
بررسی مستقیم کد، نه فقط این فایل):

- **۵۰ روت واقعی و قابل‌ایندکس** وجود داره (بدون پنل ادمین) — نه ۲۴ تا. موارد اضافه‌شده بعد از
  این roadmap: `/events` (خرید بلیط رویداد)، `/travel/exchange` (صرافی)، ۶ صفحه‌ی تور شهری زیر
  `/travel/tour/*` (dilijan, gyumri, garni-geghard, khor-virap, sevan-lake, yerevan-city)،
  و ۱۵ مقاله‌ی وبلاگ بیشتر از چیزی که آیتم ۱۶-۲۲ بالا اسم می‌بره.
- **آیتم ۱۴ («ویزای مولتی روسیه») دیگه معتبر نیست:** `VisaRussiaMulti.jsx` که اونجا اشاره شده،
  بعداً (کامیت‌های ۵ سپتامبر) retire و محتواش ادغام شد تو `/visa/russia/business`. خود
  `src/views/service/VisaRussiaMulti.jsx` الان کد یتیمه (هیچ‌جا import نمی‌شه) و
  `app/visa/russia/multi/page.jsx` فقط یه `redirect()` به business هست، نه صفحه‌ی محتوایی.
- **`public/sitemap.xml` با روت‌های واقعی sync شد** (کامیت `0e33d6a1b`) — ۸ صفحه‌ی جاافتاده
  اضافه شد، `/visa/russia/multi` (که noindex+redirect بود) حذف شد.
- **کد یتیم شناسایی‌شده که هنوز حذف نشده** (منتظر تأیید نهایی کاربره): کل کتابخونه‌ی shadcn/ui
  (۴۴ فایل، هیچ‌جا import نمی‌شه)، بقایای سیستم رزرو/جست‌وجوی قدیمی (`BookingCalendar`,
  `HotelSearch`, `FlightSearch`, ...)، کامپوننت‌های صفحه‌ی اصلی قدیمی (`TicketSection`,
  `NewsSlider`, ...)، `src/views/service/DynamicService.jsx`, `VisaRussiaMulti.jsx`,
  `src/lib/PageNotFound.jsx` (فیکس شد ولی هنوز یتیمه).
- **نکته امنیتی جدی، خارج از دامنه‌ی روتینگ:** `src/components/task.tsx` (پنل ادمین رزرو، تنها
  مصرف‌کننده‌ی `/admin/dashboard/task`) یه پسورد هاردکد شده (`M@Caspian`) رو مستقیم تو کد کلاینت
  به یه Supabase edge function می‌فرسته — قابل‌مشاهده برای هرکسی که ریپو یا view-source رو ببینه.
  باید فوری چرخانده بشه و احراز هویت به سمت سرور منتقل بشه.
- سایت `app/not-found.jsx` (روش استاندارد Next.js برای صفحه‌ی ۴۰۴ سفارشی) نداره؛ از ۴۰۴ پیش‌فرض
  و بدون استایل Next.js استفاده می‌کنه.

برای جزئیات کامل و لیست دقیق فایل‌ها، تاریخچه‌ی چت مربوطه رو ببین (این جلسه، ۸ سپتامبر ۲۰۲۶).
